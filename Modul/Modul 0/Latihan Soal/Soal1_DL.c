...
void printElement(DListNode *elem){
    printf("%d\n", elem->data);
}

void reverseEach(DListNode *elem){
...
void forEachElement(DoublyList *list, void (*func)(DListNode *)){
    if(!isEmpty(list)){
        DListNode *tmp = list->head;
...
int main(){
    ...
    forEachElement(&dl, printElement);
    forEachElement(&dl, reverseEach);
    forEachElement(&dl, printElement);
    forEachElement(&dl, sumDigit);
    forEachElement(&dl, printElement);
}