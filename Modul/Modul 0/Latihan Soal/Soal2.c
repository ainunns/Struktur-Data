...
void insertAt(DynamicArray *dArr, int index, int val){
    if(isEmpty(dArr) || index >= dArr->_size){
        push_back(dArr, val);
        return;
    }
    if(dArr->_size + 1 > dArr->_capacity){
    ...
    dArr->_size++;
    for(int i = dArr->_size; i > index; i--){
        dArr->_arr[i] = dArr->_arr[i-1];
    }
    dArr->_arr[index] = val;
}

void removeAt(DynamicArray *dArr, int index){
    if(!isEmpty(dArr)){
        for(int i = index; i+1 < dArr->_size; i++){
            dArr->_arr[i] = dArr->_arr[i+1];
        }
        dArr->_size--;
    }
